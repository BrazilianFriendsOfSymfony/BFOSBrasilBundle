        ...
        switch ($constraint->aceitar) {
            case 'dddtelefone':
                if ($this->checkDddTelefone($value, $constraint->aceitar_formatado) == 1) {
                    if ($constraint->aceitar_formatado) {
                        $this->context->addViolation($constraint->message_dddtelefone_formatado, array('{{ value }}' => $value));
                    } else {
                        $this->context->addViolation($constraint->message_dddtelefone, array('{{ value }}' => $value));
                    }
                    return false;
                } elseif ($this->checkDddTelefone($value, $constraint->aceitar_formatado) == 2) {
                    $this->context->addViolation($constraint->message_ddd);
                    return false;
                }
                break;
            ...
            default:
                if (!$this->checkTelefone($value, $constraint->aceitar_formatado)) {
                    if ($constraint->aceitar_formatado) {
                        $this->context->addViolation($constraint->message_telefone_formatado, array('{{ value }}' => $value));
                    } else {
                        $this->context->addViolation($constraint->message_telefone, array('{{ value }}' => $value));
                    }
                    return false;
                }
                break;
